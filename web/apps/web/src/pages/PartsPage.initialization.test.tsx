...
const state = vi.hoisted<{
  freshness: string; pending: boolean; error: string | null; prepare: ReturnType<typeof vi.fn>;
}>(() => ({
  freshness: "current", pending: false, error: null,
  prepare: vi.fn(),
}));
...
vi.mock("../context/PlanWorkspaceContext", () => ({ usePlanWorkspace: () => ({
  ...
  loading: false, draftLoading: false, saving: false,
...
beforeEach(() => {
  state.freshness = "current"; state.pending = false; state.error = null;
