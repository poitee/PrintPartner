...
try {
  ...
  const subheadings = () => page.locator('[aria-label="Checkoff worklist"]').getByRole("heading", { level: 4 }).allTextContents();
  ...
  assert.deepEqual(await subheadings(), Array.from({ length: 7 }, () => ["Folder-0", "Folder-1", "Folder-2", "Folder-3", "Folder-4"]).flat());
  ...
  assert.deepEqual(await subheadings(), Array.from({ length: 5 }, () => sourceHeadings).flat());
  ...
  assert.deepEqual(await subheadings(), ["Folder-1"]);
  ...
  assert.deepEqual(await headings(), []);
  assert.deepEqual(await subheadings(), []);
