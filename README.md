...
An active Build owns its Sources, Plan, Production work, and Checkoff history. The Library is shared across Builds. See [`CONTEXT.md`](.agents/CONTEXT.md) for the exact vocabulary and [ADR 0001](docs/adr/0001-model-build-workflow-as-preparation-and-making.md) for why the model is a loop and not four steps.
