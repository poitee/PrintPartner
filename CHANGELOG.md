...
- **Invite-only Board** — signed-in GitHub and email users post a frozen
  ...
  the invite host.
- **Reference import** — a recipient maps each shared source to a Library
  Source they already have. Included parts show File required, Revision
  unverified, or Ready. A printable Build is created only when every included
  part is Ready, and repeating that mapping does not create a second Build.
  Selected Library sources can be exported as a references-only collection.
